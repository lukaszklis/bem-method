...
* Объединить отдельные [файлы-технологии](../filestructure/filestructure.ru.md#Реализация-блока-разделяется-на-отдельные-файлы), разложенные по файловой структуре проекта.
* Подключить в проект только нужные [БЭМ-сущности](../key-concepts/key-concepts.ru.md#БЭМ-сущность).
* Учесть порядок их подключения.
* Преобразовать исходные файлы.
...
Список [БЭМ-сущностей](../key-concepts/key-concepts.ru.md#БЭМ-сущность) используемых на странице называется [декларацией](../declarations/declarations.ru.md). Инструмент сборки, основываясь на данных из декларации, определяет список сущностей, попадающих в проект.
