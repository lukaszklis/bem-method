...
Чтобы получить из отдельных файлов технологий общие файлы для всего проекта (`project.css`, `project.js`, `project.bemhtml.js` и т. п.) используется сборка. Такие наборы файлов, полученные в результате сборки, в БЭМ-методологии принято называть **бандлами**.
...
* Преобразовывает исходные файлы.
...
Список БЭМ-сущностей используемых на странице называется [декларацией](../declarations/declarations.ru.md). Инструмент сборки, основываясь на данных из декларации, определяет список сущностей, попадающих в проект.

Задача декларации — определить, что и в каком порядке подключать в бандлы. Ниже приведен пример сборки БЭМ-проекта.

**Пример**

![Сборка БЭМ-проекта](https://rawgit.com/bem-site/bem-method/godfreyd-methodology-deps/method/build/build__decl.svg#)

В данном примере на странице используются три блока (`button`, `input`, `langswitch`). Не обязательно включать в сборку всю библиотеку. Достаточно составить список (Declaration) БЭМ-сущностей, используемых на странице. На основании этого списка в финальный файл (`project.css`) попадут только перечисленные блоки.
